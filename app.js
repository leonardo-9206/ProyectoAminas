    ...
    function loadState() {
        ...
        if (saved) {
            try {
                ...
                // Fix para nextPatientId si venía de una versión anterior
                if (!appState.nextPatientId || isNaN(appState.nextPatientId)) {
                    appState.nextPatientId = appState.patients.length > 0 
                        ? Math.max(...appState.patients.map(p => p.id)) + 1 
                        : 2;
                }
